...
$('#itemForm').addEventListener('submit', async (event) => { event.preventDefault(); const data = new FormData(event.target); const file = data.get('image'); const image = file?.size ? await readImage(file) : ''; items.unshift({ id: Date.now(), name: data.get('name').trim(), location: data.get('location').trim(), code: data.get('code').trim(), status: 'good', image, memo: data.get('memo').trim() }); saveItems(); closeModal('#formModal'); render(); showToast('アイテムを登録しました'); });
