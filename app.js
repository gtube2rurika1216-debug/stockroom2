...
function render() {
  items.sort((first, second) => (Number(second.createdAt || second.id) || 0) - (Number(first.createdAt || first.id) || 0));
...
async function initializeCloudSync() {
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    stopCloudSync = cloudItems.onSnapshot((snapshot) => {
      if (!snapshot.empty) {
        items = snapshot.docs.map((doc) => doc.data()).sort((first, second) => (Number(second.createdAt || second.id) || 0) - (Number(first.createdAt || first.id) || 0));
        render();
